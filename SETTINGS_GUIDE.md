...
TokenPilot이 Claude Code, Gemini/Antigravity, Codex 사용량과 Grok Build local context metadata를 가져오는 방법 및 각 설정의 신뢰도/보안 경계를 설명합니다.
...
| Gemini / Antigravity | exact local metadata files | `~/Library/Application Support/TokenPilot/antigravity-statusline.json`, `~/.gemini/telemetry.log` | strict file-name parsing | 없음 |
...
| Grok Build | local context metadata | `~/.grok/sessions/**/signals.json` | 제한된 로컬 JSON 파싱 | 없음 |

기본값에서는 네트워크 usage 요청이나 provider subprocess 호출이 없습니다. Grok Build metadata, Gemini/Antigravity local metadata, 그리고 Codex local activity는 모두 local context 정보이며 provider quota나 API billing과 비교할 수 없습니다. Codex Limit Hints Connector와 Telegram/Discord test/alert send는 사용자가 직접 켠 경우에만 동작합니다.
...
## 2. Gemini / Antigravity local metadata

### 허용 파일

TokenPilot은 다음의 **정확한 파일명**만 읽습니다.
...
`antigravity-statusline.json`은 Antigravity CLI custom `statusLine`이 기록한 context-window token metadata용 파일입니다. `telemetry.log`는 legacy Gemini telemetry용 파일입니다. TokenPilot은 chat/session JSON, 임시 디렉터리, history 디렉터리 및 그 밖의 Gemini JSON 파일을 열거나 파싱하기 전에 거부합니다.
...
이 값은 로컬 context metadata이며 Gemini provider quota나 API billing이 아닙니다.
...
## 5. Grok Build local context metadata

TokenPilot은 `~/.grok/sessions/**/signals.json`에서 Grok Build의 남은 로컬 context metadata만 읽습니다. 이 source는 subscription quota, provider quota, 또는 API billing을 읽거나 추정하지 않습니다.

### 허용 범위와 안전 제한

- 파일명은 정확히 `signals.json`이어야 합니다.
- symlink는 거부합니다.
- 각 파일은 최대 256 KiB까지만 읽습니다.
- 한 번의 scan에서 최대 120개 파일만 처리합니다.
- context 관련 **숫자 필드만** 파싱합니다. 문자열, prompt, response, credential, OAuth 또는 session 내용은 사용하지 않습니다.
- **로컬 컨텍스트 기능**은 `~/.grok/auth.json`과 다른 Grok credential 파일을 읽지 않습니다.

Grok Build local context 값은 provider quota나 API billing과 직접 비교할 수 없으며 export에서도 local context metadata로만 취급합니다.

### 별도: 실험적 / 비공식 OAuth 주간 사용량 (기본 OFF)

로컬 컨텍스트와 **별개**인 기본 OFF EXPERIMENTAL/UNOFFICIAL 기능입니다. 명시적 동의 후에만 고정 경로 `~/.grok/auth.json`에서 선택된 access token과 expiry만 읽고, 고정 weekly billing 요청을 한 번 수행하며, 토큰은 메모리에만 유지하고 표시·로그·저장·진단·export 하지 않습니다. Manual weekly truth가 experimental OAuth presentation보다 우선합니다.
...
| `geminiTelemetryLogPath` | 정확한 Antigravity statusLine JSON 또는 Gemini telemetry log 파일 경로 |
...
3. legacy source를 쓰는 경우 정확한 `~/.gemini/telemetry.log` 파일이 사용량 이벤트를 포함하는지 확인
4. chat/session JSON, `tmp`, `history` 및 다른 Gemini JSON 파일은 지원하지 않음
...
- Gemini/Antigravity: 정확한 허용 파일에 Antigravity `context_window` token metadata 또는 legacy `gemini_cli.api_response` 이벤트가 있는지 확인
- Grok Build: `~/.grok/sessions/**/signals.json`에 숫자 context field가 있는지 확인. 이 값은 quota나 billing이 아님
