...
TokenPilot이 Claude Code, Gemini CLI, Codex 사용량을 가져오는 방법과 각 설정의 신뢰도/보안 경계를 설명합니다.
...
| Gemini CLI | telemetry log + session token object | `~/.gemini/telemetry.log`, `~/.gemini/**` | 로그/JSON 파싱 | 없음 |
...
| Codex Limit Hints Connector | Codex CLI app-server limit-hints RPC | 로컬 `codex app-server` + JSON-RPC `account/rateLimits/read` | **명시 opt-in 로컬 app-server/RPC 요청** | 사용자가 켠 경우만 |
...
TokenPilot은 `gemini_cli.api_response` 이벤트만 사용량 이벤트로 처리합니다.
...
Codex는 Claude statusline이나 Gemini telemetry처럼 안정적으로 문서화된 공식 사용량 파일이 없습니다. TokenPilot은 기본적으로 다음 범위만 사용합니다.
...
| `geminiTelemetryLogPath` | Gemini telemetry log 경로 |
...
### `Select telemetry.log`가 계속 표시

1. Gemini CLI telemetry가 활성화되어 있는지 확인
2. `~/.gemini/telemetry.log`가 존재하는지 확인
3. 다른 경로라면 Settings에서 파일을 선택
...
- Gemini: `gemini_cli.api_response` 이벤트 존재 여부 확인
