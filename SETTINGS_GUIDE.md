...
| Grok / xAI API | Management API key 존재 상태 + 선택 team ID | TokenPilot Keychain item, 로컬 설정(team ID) | 설정 기반만 준비, **xAI HTTP 없음** | 없음 |

기본값에서는 네트워크 usage 요청이 없습니다. Grok / xAI API는 disabled/not-configured neutral 상태이며 xAI HTTP 요청을 보내지 않습니다. Codex Limit Hints Connector와 Telegram/Discord test/alert send는 사용자가 직접 켠 경우에만 외부 요청을 보냅니다.
...
## 5. Grok / xAI API — no-network foundation

Grok / xAI API(`.xai`)는 현재 설정 기반만 준비합니다. 기본값은 disabled/not-configured neutral 상태이며, 설정을 저장해도 production에서 xAI HTTP 요청을 보내지 않습니다.
...
xAI 공식 Management 인증 문서에서 Management-key transport가 명확해질 때까지 상태는 `Management authentication unconfirmed`로 취급합니다. 이 기간에는 xAI endpoint를 호출하지 않습니다. 향후 네트워크 기능을 추가하더라도 사용자가 명시적으로 켠 경우에만 동작해야 하며, 호출 가능한 endpoint allowlist를 코드/문서에 함께 둬야 합니다.
...
### Grok / xAI API가 `not configured` 또는 `Management authentication unconfirmed`인 경우

1. 기본 상태입니다. xAI provider는 production에서 xAI HTTP 요청을 보내지 않습니다.
...
4. 공식 xAI Management 인증 문서가 Management-key transport를 명확히 할 때까지 endpoint 호출은 비활성입니다.
