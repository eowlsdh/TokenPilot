...
private enum TokenPilotCLIRunner {
    static func run(arguments: [String]) async -> Int32 {
        ...
        case .success(.help):
            ...
            return 0
        case .success(.summary(let period, let since, let until, let days, let timeZone, let includesBreakdown, let project, let sections, let weekStartDay, let includesCost, let includesJSON, let instances, let includesCSV)):
            ...
            return 0
        case .success(.stats(let period, let since, let until, let days, let includesCost, let timeZone, let includesBreakdown, let project, let includesJSON, let weekStartDay, let sections, let instances, let includesCSV)):
            let settings = TokenPilotSettingsStore().load()
            let events = UsageHistoryStore().loadEvents()
            let calendar = cliCalendar(for: timeZone)
            let effectiveSince = weekStartDay.map { TokenPilotCLIService.weekStartDate($0, calendar: calendar) } ?? since
        ...
        case .success(.report(let period, let format, let since, let until, let days, let includesCost, let timeZone, let includesBreakdown, let project, let sections, let weekStartDay, let instances)):
            ...
            return 0
        case .success(.audit(let includesJSON, let since, let until, let days, let timeZone, let project, let sections, let includesCSV, let instances)):
            let events = UsageHistoryStore().loadEvents()
            let calendar = cliCalendar(for: timeZone)
            if includesCSV {
                print(
