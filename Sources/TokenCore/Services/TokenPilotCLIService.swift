...
public enum TokenPilotCLICommand: Equatable, Sendable {
    ...
    case audit(includesJSON: Bool = false, since: Date? = nil, until: Date? = nil, days: Int? = nil, timeZone: TimeZone? = nil, project: String? = nil, sections: [HistoryPeriod]? = nil, includesCSV: Bool = false)
...
public enum TokenPilotCLIService {
    ...
    private static func parseAudit(_ flags: [String]) -> Result<TokenPilotCLICommand, TokenPilotCLIError> {
        ...
        return .success(.audit(includesJSON: includesJSON, since: since, until: until, days: days, timeZone: timeZone, project: project, sections: sections, includesCSV: includesCSV))
    }
    ...
    private static func parseSummary(_ flags: [String]) -> Result<TokenPilotCLICommand, TokenPilotCLIError> {
        ...
        return .success(.summary(period: period, since: since, until: until, days: days, timeZone: timeZone, includesBreakdown: includesBreakdown, project: project, sections: sections, weekStartDay: weekStartDay, includesCost: includesCost, includesJSON: includesJSON, instances: instances, includesCSV: includesCSV))
    }
    ...
    private static func parseBlocks(_ flags: [String]) -> Result<TokenPilotCLICommand, TokenPilotCLIError> {
        ...
        return .success(.blocks(includesJSON: includesJSON, active: active, recent: recent, timeZone: timeZone, since: since, until: until, days: days, includesCSV: includesCSV))
    }
    ...
    public static var helpText: String {
        ...
        Usage:
          ...
          TokenPilot audit [--json|--csv] [--sections today,last7Days,thisMonth]
        ...
        rows for spreadsheets (toktrack stats --csv style), --sections (JSON only) emits summaries for
        ...
        emits coverage for several periods in one envelope. blocks lists the
        ...
        structured JSON, and --csv emits the same blocks as rows for spreadsheets. Exports, reports,
        and audits never
