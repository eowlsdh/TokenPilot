...
public enum TokenPilotCLIService {
    ...
    private static func parseReport(_ flags: [String]) -> Result<TokenPilotCLICommand, TokenPilotCLIError> {
        ...
        return .success(.report(period: period, format: format, since: since, until: until, days: days, includesCost: includesCost, timeZone: timeZone, includesBreakdown: includesBreakdown, project: project, sections: sections, weekStartDay: weekStartDay, instances: instances, provider: provider, model: model))
    }
    ...
    private static func parseStats(_ flags: [String]) -> Result<TokenPilotCLICommand, TokenPilotCLIError> {
        ...
        return .success(.stats(period: period, since: since, until: until, days: days, includesCost: includesCost, timeZone: timeZone, includesBreakdown: includesBreakdown, project: project, includesJSON: includesJSON, weekStartDay: weekStartDay, sections: sections, instances: instances, includesCSV: includesCSV, includesMarkdown: includesMarkdown, provider: provider, model: model, sort: sort))
    }
    ...
    public static var helpText: String {
        ...
        to one provider (ccusage --provider style), and --model restricts it to one model name
        (ccusage --model style). --breakdown adds a per-day, per-model
